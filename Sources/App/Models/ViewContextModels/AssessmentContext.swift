...
final class AssessmentContext: Content {
    ...
    var embedCode: String {
        """
        <iframe src="\(urlForNewInstance)" style="width:100%; height:800px;display:block; margin-left:auto; margin-right:auto;" frameborder="0" scrolling="no" id="MQWcontent" name="MQWcontent"></iframe><script type="text/javascript"> function getJsonFromUrl() {    var query = location.search.substr(1); var result = {}; query.split("&").forEach(function(part) {  var item = part.split("=");  result[item[0]] = decodeURIComponent(item[1]); }); return result; } function receiveMessage(event) {   if (! isNaN(event.data)) {  var iframe = document.getElementById('MQWcontent');  iframe.style.height = iframe.height = event.data + "px"; }  document.body.scrollTop = document.documentElement.scrollTop = 0; }  if(typeof window.addEventListener != 'undefined'){  window.addEventListener('message', receiveMessage, false); } else if(typeof window.attachEvent != 'undefined') {  window.attachEvent('onmessage', receiveMessage); }</script>
        """
    }
